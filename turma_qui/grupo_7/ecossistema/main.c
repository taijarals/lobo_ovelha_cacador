...
ResultadoSimulacao executar_simulacao(Mundo *m, int max_rodadas);
...
/* ============================================================
 * FUNCAO: Verifica se existe uma arvore no caminho entre duas posicoes
 * 
 * Esta funcao implementa a logica correta de protecao da arvore:
 * A arvore protege o lobo se estiver entre o lobo e o cacador,
 * formando uma barreira natural.
 * ============================================================ */

static int existe_arvore_no_caminho(Celula **tab, int x1, int y1, int x2, int y2, int N) {
    /* Verifica se as posicoes sao validas */
    if (!posicao_valida(x1, y1, N) || !posicao_valida(x2, y2, N)) {
        return 0;
    }
    
    /* Se estiverem na mesma linha (mesmo y) */
    if (y1 == y2) {
        int min_x = (x1 < x2) ? x1 : x2;
        int max_x = (x1 > x2) ? x1 : x2;
        
        /* Verifica se ha uma arvore entre as duas posicoes */
        for (int x = min_x + 1; x < max_x; x++) {
            if (tab[x][y1].tipo == CELULA_ARVORE) {
                return 1;  /* Encontrou uma arvore no caminho */
            }
        }
    }
    
    /* Se estiverem na mesma coluna (mesmo x) */
    if (x1 == x2) {
        ...
        for (int y = min_y + 1; y < max_y; y++) {
            if (tab[x1][y].tipo == CELULA_ARVORE) {
                return 1;  /* Encontrou uma arvore no caminho */
            }
        }
    }
    
    return 0;  /* Nenhuma arvore no caminho */
}
...
static void agir_cacador(Mundo *m, int idx) {
    ...
    if (alvo->tipo == TIPO_LOBO && tem_municao(e)) {
        /* ============================================================
         * PROTECAO DA ARVORE: Verifica se existe uma arvore no caminho
         * entre o cacador e o lobo. Se houver, o lobo esta protegido.
         * ============================================================ */
        if (existe_arvore_no_caminho(m->tabuleiro, e->x, e->y, alvo->x, alvo->y, m->N)) {
            return;  /* Arvore bloqueia o tiro do cacador */
        }
...
ResultadoSimulacao executar_simulacao(Mundo *m, int max_rodadas) {
    ...
    for (int rodada = 1; rodada <= max_rodadas; rodada++) {
        ...
        if (ov == 0) {
            r.motivo_fim = FIM_OVELHAS_EXTINTAS;
            r.total_rodadas = rodada;
            r.ovelhas_final = 0; r.lobos_final = lo; r.cacadores_final = ca;
            return r;
        }
        if (lo == 0) {
            r.motivo_fim = FIM_LOBOS_EXTINTOS;
            r.total_rodadas = rodada;
            r.ovelhas_final = ov; r.lobos_final = 0; r.cacadores_final = ca;
            return r;
        }
        if (ca == 0) {
            r.motivo_fim = FIM_CACADORES_EXTINTOS;
            r.total_rodadas = rodada;
            r.ovelhas_final = ov; r.lobos_final = lo; r.cacadores_final = 0;
            return r;
        }
    }
    ...
    r.ovelhas_final = ov; r.lobos_final = lo; r.cacadores_final = ca;
    return r;
}
...
int main(void) {
    ...
    if (scanf("%d", &max_rodadas) != 1 || max_rodadas < 1) {
